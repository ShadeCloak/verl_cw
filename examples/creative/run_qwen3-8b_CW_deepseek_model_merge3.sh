...
#
# This version includes model merge functionality: every 5 steps, the current model is merged
# with the initial model in a 1:1 ratio to prevent catastrophic forgetting.
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="qwen3_8b_function_rm_model_merge3_$TIMESTAMP" \
    trainer.n_gpus_per_node=8 \
    trainer.nnodes=1 \
    trainer.save_freq=5 \
    trainer.test_freq=5 \
    trainer.total_epochs=2 \
    +trainer.enable_model_merge=True \
    +trainer.model_merge_freq=5 \
    +trainer.model_merge_initial_path=/workspace/qingnan/model/Qwen3-8B \
    +trainer.model_merge_ratio=0.9 $@