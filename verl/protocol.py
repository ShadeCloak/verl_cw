...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        if data:
            ...
            all_metrics = []
            # Statistical fields that should be aggregated across workers
            stat_fields = {
                'rm_output_length_min': [],
                'rm_output_length_max': [],
                'rm_output_length_mean': [],
                'rm_extraction_method_counts': [],
                'num_invalid_scores': []
            }
            # List fields that should be concatenated from all workers
            all_failure_cases = []
            ...
            for d in data:
                for k, v in d.meta_info.items():
                    ...
                    elif k == "rm_failure_cases":
                        # Collect failure cases from all workers
                        if v is not None:
                            if isinstance(v, list):
                                all_failure_cases.extend(v)
                            else:
                                all_failure_cases.append(v)
                    elif k in stat_fields:
                        # Collect statistical values from all workers
                        stat_fields[k].append(v)
                    else:
            ...
            if stat_fields['rm_output_length_min']:
                merged_meta_info['rm_output_length_min'] = int(min(stat_fields['rm_output_length_min']))
            if stat_fields['rm_output_length_max']:
                merged_meta_info['rm_output_length_max'] = int(max(stat_fields['rm_output_length_max']))
            if stat_fields['rm_output_length_mean']:
                # Take the mean of means (this is an approximation, but reasonable for monitoring)
                merged_meta_info['rm_output_length_mean'] = float(np.mean(stat_fields['rm_output_length_mean']))
            if stat_fields['rm_extraction_method_counts']:
                # Merge dictionaries by summing counts
                from collections import Counter
                merged_counts = Counter()
                for counts_dict in stat_fields['rm_extraction_method_counts']:
                    merged_counts.update(counts_dict)
                merged_meta_info['rm_extraction_method_counts'] = dict(merged_counts)
            if stat_fields['num_invalid_scores']:
                # Sum up invalid scores from all workers
                merged_meta_info['num_invalid_scores'] = int(sum(stat_fields['num_invalid_scores']))

            # Merge failure cases from all workers
            if all_failure_cases:
                merged_meta_info['rm_failure_cases'] = all_failure_cases
    ...
    def reorder(self, indices):
        ...
        # Convert list values to numpy arrays before indexing
        self.non_tensor_batch = {
            key: np.array(val)[indices_np] if isinstance(val, list) else val[indices_np]
            for key, val in self.non_tensor_batch.items()
        }
