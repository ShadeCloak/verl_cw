...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def merge_with_initial_model(self, initial_model_path: str, merge_ratio: float = 0.5):
        """Merge current actor model with initial model.

        Args:
            initial_model_path: Path to the initial model checkpoint
            merge_ratio: Ratio for merging (0.5 means 1:1, current_weight * merge_ratio + initial_weight * (1 - merge_ratio))
        """
        from verl.utils.logger import log_with_rank

        # Only perform merge for actor
        assert self._is_actor, "Model merge is only supported for actor"

        log_with_rank(
            f"[rank-{self.rank}]: Starting model merge with initial model at {initial_model_path}, merge_ratio={merge_ratio}",
            rank=dist.get_rank(),
            logger=logger,
            log_only_rank_0=True,
        )
        ...
        with FSDP.state_dict_type(
            ...
            current_state_dict = self.actor_module_fsdp.state_dict()
        ...
        if dist.get_rank() == 0:
            ...
            initial_state_dict = {}
            safetensors_files = [f for f in os.listdir(initial_model_path) if f.endswith('.safetensors')]

            if safetensors_files:
                # Load from safetensors
                log_with_rank(
                    f"[rank-{self.rank}]: Loading initial model from safetensors",
                    rank=dist.get_rank(),
                    logger=logger,
                    log_only_rank_0=True,
                )
                for safetensors_file in safetensors_files:
                    file_path = os.path.join(initial_model_path, safetensors_file)
                    shard_dict = safetensors.torch.load_file(file_path)
                    initial_state_dict.update(shard_dict)
            else:
                # Load from pytorch_model.bin
                log_with_rank(
                    f"[rank-{self.rank}]: Loading initial model from pytorch checkpoint",
                    rank=dist.get_rank(),
                    logger=logger,
                    log_only_rank_0=True,
                )
                model_files = [f for f in os.listdir(initial_model_path) if f.startswith('pytorch_model') and f.endswith('.bin')]
                for model_file in model_files:
                    file_path = os.path.join(initial_model_path, model_file)
                    shard_dict = torch.load(file_path, map_location='cpu', weights_only=False)
                    if 'state_dict' in shard_dict:
                        shard_dict = shard_dict['state_dict']
                    initial_state_dict.update(shard_dict)

            # Convert keys if necessary
            initial_state_dict = convert_weight_keys(initial_state_dict, self.actor_module)

            # Perform 1:1 merge (average the weights)
            merged_state_dict = {}
            for key in current_state_dict.keys():
                if key in initial_state_dict:
                    # Merge: current * merge_ratio + initial * (1 - merge_ratio)
                    merged_state_dict[key] = (
                        current_state_dict[key] * merge_ratio +
                        initial_state_dict[key].to(current_state_dict[key].dtype) * (1 - merge_ratio)
                    )
                else:
                    # If key not in initial model, keep current value
                    merged_state_dict[key] = current_state_dict[key]
                    log_with_rank(
                        f"[rank-{self.rank}]: Warning: Key {key} not found in initial model, keeping current value",
                        rank=dist.get_rank(),
                        logger=logger,
                        log_only_rank_0=True,
                    )
        ...
        # Load merged state dict back to model
        with FSDP.state_dict_type(
            self.actor_module_fsdp,
            StateDictType.FULL_STATE_DICT,
            FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
        ):
            self.actor_module_fsdp.load_state_dict(merged_state_dict, strict=False)

        dist.barrier()

        log_with_rank(
            f"[rank-{self.rank}]: Model merge completed successfully",
            rank=dist.get_rank(),
            logger=logger,
            log_only_rank_0=True,
        )
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        # Handle different model architectures (similar to critic.py logic)
        # This allows using CausalLM models as reward models
        arch = model_config.architectures[0]
        use_sequence_classification = False

        if "ForCausalLM" in arch:
            model_name = arch.split("ForCausalLM")[0]
            # Try TokenClassification first, fall back to SequenceClassification
            new_arch = f"{model_name}ForSequenceClassification"
            warnings.warn(
                f"Reward model: Implicitly changing architecture from '{arch}' to '{new_arch}' "
                f"to support CausalLM models as reward models.",
                stacklevel=2
            )
            model_config.architectures[0] = new_arch
            use_sequence_classification = True
        elif "ForSequenceClassification" in arch:
            use_sequence_classification = True
        elif "ForTokenClassification" not in arch:
            raise ValueError(
                f"Unsupported reward model architecture: {arch}. "
                f"Reward model expects an architecture suitable for scoring, "
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # Try to load the model with the appropriate Auto class
            if use_sequence_classification:
                # For models that use SequenceClassification (e.g., DeepSeek-GRM)
                # When trust_remote_code=True and auto_map doesn't have SequenceClassification,
                # we need to manually import and use the class
                if trust_remote_code and hasattr(model_config, "auto_map"):
                    # Check if AutoModelForSequenceClassification is in auto_map
                    if "AutoModelForSequenceClassification" not in model_config.auto_map:
                        # Manually load the class from the local modeling file
                        import sys
                        import importlib.util
                        from pathlib import Path

                        modeling_file = Path(local_path) / "modeling_deepseek.py"
                        if modeling_file.exists():
                            # Import the modeling module
                            spec = importlib.util.spec_from_file_location("modeling_deepseek", modeling_file)
                            modeling_module = importlib.util.module_from_spec(spec)
                            sys.modules["modeling_deepseek"] = modeling_module
                            spec.loader.exec_module(modeling_module)

                            # Get the SequenceClassification class
                            model_class_name = arch.replace("ForCausalLM", "ForSequenceClassification")
                            if hasattr(modeling_module, model_class_name):
                                ModelClass = getattr(modeling_module, model_class_name)
                                reward_module = ModelClass.from_pretrained(
                                    pretrained_model_name_or_path=local_path,
                                    config=model_config,
                                ...
                                raise ValueError(f"Model class {model_class_name} not found in {modeling_file}")
                        else:
                ...
                # For models that use TokenClassification
