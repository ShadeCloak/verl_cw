...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        # Build id2score mapping, excluding invalid samples
        for i in range(bsz):
            # Skip invalid samples when computing group statistics
            if invalid_mask is not None and invalid_mask[i]:
                continue
            id2score[index[i]].append(scores[i])

        # Compute mean and std for each group (only using valid samples)
        for idx in id2score:
        ...
        # Normalize all samples (including invalid ones)
        for i in range(bsz):
            if invalid_mask is not None and invalid_mask[i]:
                # Set advantage to 0 for invalid samples
                scores[i] = torch.tensor(0.0)
            else:
                # Normal GRPO normalization for valid samples
...
@register_adv_est("similarity_group_grpo")
def compute_similarity_group_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    similarity_group_labels: list,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    config: Optional[AlgoConfig] = None,
    invalid_mask: Optional[np.ndarray] = None,
    ...
    """
    Compute advantage for GRPO with similarity-based sub-grouping for creative writing.

    This function implements a two-level grouping strategy:
    1. First level: Group by prompt (uid)
    2. Second level: Group by response similarity within each prompt

    Only responses in the same similarity sub-group are compared for reward normalization.
    Single-member groups get advantage = 0 (as they would after normalization anyway).

    This preserves diversity in creative writing by not comparing dissimilar responses.

    Args:
        token_level_rewards: (bs, response_length) token-level rewards
        response_mask: (bs, response_length) mask for valid tokens
        index: (bs,) prompt UIDs for first-level grouping
        similarity_group_labels: (bs,) combined group labels in format "{uid}_{sim_group}"
        epsilon: Small value for numerical stability
        norm_adv_by_std_in_grpo: Whether to normalize by std (default True)
        config: Algorithm configuration

    Returns:
        advantages: (bs, response_length) normalized advantages
        returns: (bs, response_length) same as advantages for GRPO

    Example:
        For a batch with 2 prompts, each with 8 responses:
        - Prompt 0: responses split into 3 similarity groups [0, 0, 1, 1, 1, 2, 2, 2]
        - Prompt 1: responses split into 2 similarity groups [0, 0, 0, 0, 1, 1, 1, 1]
        ...
        Normalization happens within each sub-group independently.
    """
    scores = token_level_rewards.sum(dim=-1)  # (bs,)

    # Convert similarity_group_labels to contiguous group indices
    # This handles the "{uid}_{sim_group}" format
    g = as_torch_index(similarity_group_labels, device=scores.device)

    with torch.no_grad():
        # Handle invalid samples: exclude them from group statistics computation
        if invalid_mask is not None:
            invalid_mask_torch = torch.from_numpy(invalid_mask).to(scores.device)
            # Create a mask for valid samples
            valid_mask = ~invalid_mask_torch
            # Only compute stats on valid samples
            valid_scores = scores.clone()
            valid_scores[invalid_mask_torch] = float('nan')  # Mark invalid as NaN for group_mean_std

            # Compute per-group mean and std (will ignore NaN values if group_mean_std supports it)
            # For now, we'll use a manual approach
            from collections import defaultdict
            group_scores = defaultdict(list)
            for i in range(len(scores)):
                if not invalid_mask[i]:  # Only include valid samples
                    group_scores[g[i].item()].append(scores[i])

            # Compute mean and std for each group
            mean_dict = {}
            std_dict = {}
            count_dict = {}
            for group_id, group_vals in group_scores.items():
                if len(group_vals) > 0:
                    group_tensor = torch.stack(group_vals)
                    mean_dict[group_id] = group_tensor.mean()
                    std_dict[group_id] = group_tensor.std() if len(group_vals) > 1 else torch.tensor(1.0)
                    count_dict[group_id] = len(group_vals)
                else:
                    mean_dict[group_id] = torch.tensor(0.0)
                    std_dict[group_id] = torch.tensor(1.0)
                    count_dict[group_id] = 0

            # Create tensors for broadcasting
            mean_g = torch.zeros_like(scores)
            std_g = torch.ones_like(scores)
            count_g = torch.zeros(g.max().item() + 1, device=scores.device)

            for i in range(len(scores)):
                group_id = g[i].item()
                if group_id in mean_dict:
                    mean_g[i] = mean_dict[group_id]
                    std_g[i] = std_dict[group_id]
                    count_g[group_id] = count_dict[group_id]
        else:
            # Original behavior: compute per-group mean and std on all samples
            mean_g, std_g, count_g = group_mean_std(scores, g, eps=epsilon)

        # Identify single-member groups (count == 1)
        # These will get advantage = 0
        single_member_groups = count_g <= 1

        # Compute normalized advantages
        if norm_adv_by_std_in_grpo:
            scalars = (scores - mean_g) / (std_g + epsilon)
        else:
            scalars = scores - mean_g

        # Set single-member group advantages to 0
        # Find which samples belong to single-member groups
        single_member_mask = single_member_groups[g]
        scalars[single_member_mask] = 0.0

        # Set invalid sample advantages to 0
        if invalid_mask is not None:
            scalars[invalid_mask_torch] = 0.0
